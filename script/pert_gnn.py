...
tr2data = torch.load(__path__ + "processed/tr2data.pt")
# sample 100000 traces
tr2data = {tr: tr2data[tr] for tr in list(tr2data.keys())[:100000]}
runtime2graph = torch.load(__path__ + f"processed/runtime2{args.graph_type}graph_map.pt")
entry2runtimes = load(__path__ + "processed/entry2runtimes.joblib")
resource_df = pd.read_csv(__path__ + "processed/processed_resource_df.csv", engine="pyarrow")
...
if os.path.exists(__path__ + f"processed/full_{args.graph_type}_data_list.pt"):
    data_list = torch.load(__path__ + f"processed/full_{args.graph_type}_data_list.pt")
else:
    print("Getting data list...")
    data_list = get_data_list(tr2data, entry2runtimes, runtime2graph)
    torch.save(data_list, __path__ + f"processed/full_{args.graph_type}_data_list.pt")
