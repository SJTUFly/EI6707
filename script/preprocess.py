...
def get_df():
    if os.path.isfile("processed/processed_df.csv") and os.path.isfile(
        ...
        df = pd.concat(
            (
                pd.read_csv(path + f"/data/MSCallGraph/{f}", index_col=0, engine="pyarrow")
                # .drop("Unamed: 0", axis=1)
                .replace(np.nan, "nan")
                for f in tqdm(os.listdir(path + "/data/MSCallGraph"))
                if f.endswith(".csv")
            ),
            ignore_index=True,
        ).drop_duplicates()
        ...
        resource_df = pd.concat(
            pd.read_csv(path + f"/data/MSResource/{f}", engine="pyarrow")
            # .drop("Unnamed: 0", axis=1)
            .loc[
                :,
                ["timestamp", "msname", "instance_cpu_usage", "instance_memory_usage"],
            ]
            for f in tqdm(os.listdir(path + "/data/MSResource"))
            if f.endswith(".csv")
        )
